...
为了具体说明 `Dolphin` 类中 `click` 方法的行为——换句话说，为了验证这个方法可用——多个 `it` 例子可以用 `describe` 函数组织成为一个群。把相同的例子组织在一起能更方便阅读：
...
例子群不仅是它包含的例子更清晰易懂，还有助于在群里共享配置和卸载的代码。
