# Quick Examples와  Example 그룹으로 구성된 테스트

Quick은 **examples** 와 **example groups**를 정의하기 위해 특수한 구문을 사용합니다.

*[XCTest를 사용한 효과적인 테스트 : Arrange, Act, 그리고 Assert](ArrangeActAssert.md)* 에서,
좋은 테스트 메소드 이름이 중요하다는 것을 배웠습니다. — 테스트가 실패하기 시작하면, 우리는 응용 프로그램 코드를 수정할지, 테스트 코드를 업데이트할지를 판단하는 것이 가장 좋습니다.

Quick examples 과 example 그룹은 두 가지 용도로 사용됩니다:

1. 서술적인 테스트 이름을 작성하는 것을 장려합니다.
2. 테스트 "정돈" 단계에서 코드를 매우 단순하게  합니다.
...
이 두가지 example이 Xcode에서 실행되면, `describe` 과 `it` 함수들의 설명이 표시될 겁니다:
